...
{	# This section if required for Self-Signed Certs
	$global:LoadA9API=$true
	$global:Load3PAR=$false
...
"@  
	...
	$ModPath = $CurrentModulePath + '\HPEStorage.psd1'	
...
Function Show-HPESANArrayCommandSet
{
<#	
.SYNOPSIS
...
{	$Global:PersistArrayType = $ArrayType
	$global:LoadA9API	= $true
	if ( $ArrayType -like '3PAR' ) 
		{ 	$global:Load3PAR	= $false
		} else {}
	$global:LoadA9CLI	= $true
	$CurrentModulePath = (Get-Module HPEStorage).path
	[string]$CurrentModulePath = Split-Path $CurrentModulePath -Parent
...
#>
	...
	[string]$CurrentModulePath = Split-Path $CurrentModulePath -Parent
	$ModPath = $CurrentModulePath	
	write-host "Reloading the HPEStorage Module with the no Array Type Specific, only basic connection commands will be shown"
	$ModPath = $ModPath + '\HPEStorage.psd1'
	Import-Module $ModPath -force -scope Global
}
...
{	$Global:PersistArrayType = $ArrayType
	...
	if ($ArrayType -eq 'Alletra9000' -or $ArrayType -eq 'Primera' -or $ArrayType -eq '3Par' -or $ArrayType -eq 'AlletraMP-B10000')
			{	write-Verbose "You will be connected to a $ArrayType at the location $ArrayNameOrIPAddress"
				...
				Write-host "To View the list of commands available to you use 'Get-Command -module HPEStorage'." -ForegroundColor Green
				if ( $ArrayType -eq '3Par')
