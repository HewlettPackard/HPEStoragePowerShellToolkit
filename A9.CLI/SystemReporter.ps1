...
.PARAMETER Frequency
	Can be one of the follolwing three options;
	Hires = Select high resolution samples (5 minute intervals) for the report. This is the default setting.
...
.EXAMPLE
    PS:> Get-A9SystemReportCpgSpace -VsTime -Freqency HiRes | format-table *

	Date       Time     TimeZone Secs       PrivateBase(MB) PrivateSnap(MB) Shared(MB) Free(MB) Total(MB) UsableFree(MB) Dedup_GC(KB/s) Compact Dedup Compress DataReduce OverPro
																																										v
	----       ----     -------- ----       --------------- --------------- ---------- -------- --------- -------------- -------------- ------- ----- -------- ---------- -------
	2026-03-02 05:20:00 MST      1772454000 40486425        8162700         36465450   85114575 189049350 6.21           1.26           1.66    2.01  12.48    1.25
	2026-03-02 05:25:00 MST      1772454300 40488000        8162700         36463875   85114575 189049350 6.21           1.26           1.66    2.01  12.48    1.25
	2026-03-02 05:30:00 MST      1772454600 40489575        8162700         36462300   85114575 189049350 6.21           1.26           1.66    2.01  12.48    1.25
.NOTES
	This command requires a SSH type connection.
#>
[CmdletBinding()]
param(	[Parameter(Mandatory,ParameterSetName='AtTime')]				[switch]	$AtTime,
		[Parameter(Mandatory,ParameterSetName='VsTime')]				[SWitch]	$VsTime,
		[Parameter()]													[String]	$BeginTimeSecs,
		[Parameter()]													[String]	$EndTimeSecs,
		[Parameter(Mandatory)][ValidateSet('Hourly','Daily','HiRes')]	[String]	$Freqency,
		[Parameter()]	[ValidateSet('DOM_NAME','CPG_NAME','CPGID')]	[String]	$groupby,
...
{	$srinfocmd = "srcpgspace"
	...
	if($BeginTimeSecs)		{	$srinfocmd += " -btsecs $btsecs"	}
	if($EndTimeSecs)		{	$srinfocmd += " -etsecs $etsecs"	}
...
Function Get-A9SystemReportHistogramPhysicalDisk
{
<#
.SYNOPSIS
...
.EXAMPLE
	...
	Command displays historical histogram performance data reports for  VLUNs. 	
.EXAMPLE
    PS:> Get-A9SystemReportHistogramVLun -Frequency Hourly -btsecs -24h
...
.PARAMETER Frequency
	...
	Daily = Select daily samples for the report.
.PARAMETER Groupby
	For -attime reports, generate a separate row for <groupby> items.  Groupby must be different and one of the following:
	DOM_NAME  Domain name
	CPG_NAME  Common Provisioning Group name
	LDID      Logical disk ID
	LD_NAME   Logical disk name
...
.EXAMPLE
	...
	Command displays historical space data reports for logical disks (LDs).
.EXAMPLE
    PS:> Get-A9SystemReportLogicalDiskSpace -Hourly -begintimesecs 24h -LDName fc*
.NOTES
	This command requires a SSH type connection.
#>
[CmdletBinding()]
